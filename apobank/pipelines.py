...
class ApobankPipeline:

    # Database setup
    conn = sqlite3.connect('apobank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `apobank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM apobank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO apobank (date, title, link, content)"
